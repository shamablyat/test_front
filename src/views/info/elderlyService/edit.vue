<script setup lang="ts">
import i18n from "@/app/config/i18n";
import { useManualStore } from "@/app/config/store/Manual";
import TranslateItems from "@/components/TranslateItems.vue";
import UiParentCard from "@/components/UiParentCard.vue";
import { ISelectList } from "@/models/basic";
import ElderlyService from "@/services/info/elderlyService.service";
import { useErrorToast } from "@/utils/helpers/useErrorToast";
import { useNotification } from "@kyvg/vue3-notification";
import { ref } from "vue";
import { useRoute, useRouter } from "vue-router";
import { SubmitEventPromise } from "vuetify";

const data = ref<any>();
const loading = ref<boolean>(false);
const route = useRoute();
const manualStore = useManualStore();
manualStore.setStateSelectList();

const router = useRouter();
const notify = useNotification();
const { setError } = useErrorToast();

const saveLoading = ref<boolean>(false);
const asSelectList = ref<ISelectList[]>([]);

const get = () => {
  const id = route.params.id as string;
  loading.value = true;
  ElderlyService.Get(id).then((res) => {
    data.value = res.data;
    loading.value = false;
  });
};

const save = async (submit: SubmitEventPromise) => {
  const { valid, errors } = await submit;
  if (errors && errors.length > 0) {
    document
      // @ts-ignore
      .getElementById(errors[0].id)
      ?.scrollIntoView({
        behavior: "smooth",
        block: "center",
        inline: "center"
      });
  }

  if (valid) {
    saveLoading.value = true;

    ElderlyService.Update(data.value)
      .then(() => {
        notify.notify({
          // @ts-ignore
          text: i18n.global.t("SaveSuccess"),
          type: "success"
        });
        saveLoading.value = false;
        router.push({ name: "ElderlyService" });
      })
      .catch((err) => {
        setError(err);
        saveLoading.value = false;
      });
  }
};

ElderlyService.GetAsSelectList('?isGroup=true').then((res) => {
  asSelectList.value = res.data;
});

get();
</script>

<template>
  <div>
    <UiParentCard class="text-center" v-if="loading">
      <v-progress-circular indeterminate></v-progress-circular>
    </UiParentCard>
    <template v-if="data && !loading">
      <v-form @submit.prevent="save">
        <UiParentCard>
          <v-row class="mb-3">
            <v-col cols="12" lg="3" md="6" class="py-0">
              <form-input :label="$t('orderCode')" required v-model="data.orderCode"></form-input>
            </v-col>
            <v-col cols="12" lg="3" md="6" class="py-0">
              <form-input :label="$t('code')" v-model="data.code" required></form-input>
            </v-col>
            <v-col cols="12" lg="3" md="6" class="py-0">
              <TranslateItems
                v-model:translates="data.translates"
                column-name="short_name"
                :label="$t('shortName')"
                v-model:value="data.shortName"
                required
              />
            </v-col>
            <v-col cols="12" lg="3" md="6" class="py-0">
              <TranslateItems
                required
                v-model:value="data.fullName"
                column-name="full_name"
                v-model:translates="data.translates"
                :label="$t('fullName')"
              />
            </v-col>
          </v-row>
          <v-row class="align-center mb-6">
            <v-col cols="12" lg="3" md="6" class="py-0">
              <form-checkbox v-model="data.isGroup" :label="$t('isGroup')"></form-checkbox>
            </v-col>
            <v-col cols="12" lg="3" md="6" class="py-0">
              <form-select :list="asSelectList" v-model="data.parentId" :label="$t('parent')"></form-select>
            </v-col>
            <v-col cols="12" lg="3" md="6" class="py-0" v-if="+(route.params.id as string) != 0">
              <form-select :list="manualStore.stateSelectList" v-model="data.stateId" required :label="$t('state')"></form-select>
            </v-col>
          </v-row>

          <v-row class="align-center mb-3">
            <v-col cols="12" lg="3" class="py-0">
              <form-input v-model="data.executeOrganizationName" :required="!data.isGroup" :label="$t('organization')"></form-input>
            </v-col>
            <v-col cols="12" lg="3" class="py-0">
              <form-input v-model="data.executePeriod" :required="!data.isGroup" :label="$t('deadlineOnDay')"></form-input>
            </v-col>
            <v-col cols="12" lg="3" class="py-0">
              <form-checkbox v-model="data.isExecuteAsistent" :label="$t('isExecuteAsistent')"></form-checkbox>
            </v-col>
          </v-row>
        </UiParentCard>
        <v-row class="mt-4 px-3">
          <v-btn color="error" @click="router.back()" variant="flat" :text="$t('Back')" />
          <v-spacer />
          <v-btn color="success" :loading="saveLoading" variant="flat" type="submit" :text="$t('Save')" />
        </v-row>
      </v-form>
    </template>
  </div>
</template>
