      ...
      <v-card-title class="mb-4">
        <v-row>
          <v-col>
            ...
            <h4>{{ $t("serviceRecipientFullName") }}: {{ item.serviceRecipientFullName }}</h4>
          </v-col>
      ...
      <v-form @submit.prevent="setSocialEmployee">
        <v-card-text>
          <v-row>
            ...
            <v-col cols="12" lg="3" md="6" class="py-1">
              <form-select :list="districtList" disabled v-model="filter.districtId" :label="$t('district')" @update:model-value="setMfy" required />
            </v-col>
            <v-col cols="12" lg="3" md="6" class="py-1">
              <form-select :list="mfyList" v-model="filter.mfyId" :label="$t('mfy')" @update:model-value="getSocialPerson" required />
            </v-col>
          </v-row>
          <v-row>
            <v-col cols="12" lg="3" md="6" class="py-1 mt-3">
              <form-select
                ...
                itemValue="id"
                required
                :disabled="socialPersonList.length == 1"
              />
            </v-col>
          </v-row>
        </v-card-text>
