...
import CardInterCeptor from "./widgets/CardInterCeptor.vue";
    ...
    <div v-if="data && !loading">
      <v-row>
        <v-col cols="12">
            ...
            <div style="height: calc(100vh - 200px); overflow-y: scroll; overflow-x: hidden">
              <v-row>
                <v-col cols="12" lg="4" md="6" v-for="item in data.inTables">
                  <CardInterCeptor :item="item" @download="Download" canDownLoad />
                </v-col>
              </v-row>
            </div>
          </UiParentCard>
          ...
          <UiParentCard title="PDF" class="mt-3 text-center" v-if="iframeSrc && iframeSrc.id">
            <iframe
              :src="`${axios.defaults.baseURL}/ElderlyRegister/DownloadFile/${iframeSrc.id}`"
              width="80%"
              style="height: 800px"
              name="application"
              id="application"
