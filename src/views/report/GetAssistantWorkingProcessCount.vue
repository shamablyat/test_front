...
const isByRegionAllowed = ref(auth.permissions.includes("AssistantWorkingProcessReportByRegion"));
const isByDistrictAllowed = ref(auth.permissions.includes("AssistantWorkingProcessReportByDistrict"));
...
const refresh = () => {
  ...
  ReportService.GetAssistantWorkingProcessCount(innerFilter)
    .then((res: any) => {
...
const printReport = () => {
  ...
  ReportService.ExportAssistantWorkingProcessCount(filter)
    .then((res: any) => {
      printLoading.value = false;
      forceFileDownload(res, t("GetAssistantWorkingProcessCount"), ".xlsx");
    })
    .catch((err: any) => {
      setError(err);
      printLoading.value = false;
...
<template>
  <UiParentCard>
    <form-table-header :filter="filter" hasMenu @refresh="refresh" @clearFilter="clearFilter" @canSearch="false" :canClear="false">
      <template #menu-items>
        <v-row class="mx-2">
        ...
        <tbody v-if="!loading && rows.length > 0">
          <tr v-for="(item, index) in rows" :key="index + 'row'">
            ...
            <td colspan="2" class="text-right border">
              {{ Intl.NumberFormat(undefined, {}).format(item.workedCount) }}
            </td>
            <td colspan="2" class="text-right border">
              {{ Intl.NumberFormat(undefined, {}).format(item.notWorkedCount) }}
            </td>
            <td colspan="2" class="text-right border">
              {{ Intl.NumberFormat(undefined, {}).format(item.overWorkedCount) }}
            </td>
            <td colspan="2" class="text-right border">
              {{ Intl.NumberFormat(undefined, {}).format(item.lessWorkedCount) }}
            </td>
          ...
          <tr class="total">
            ...
            <td colspan="2" class="text-right border">
              {{ Intl.NumberFormat(undefined, {}).format(columnTotals.WorkedCount) }}
            </td>
            <td colspan="2" class="text-right border">
              {{ Intl.NumberFormat(undefined, {}).format(columnTotals.NotWorkedCount) }}
            </td>
            <td colspan="2" class="text-right border">
              {{ Intl.NumberFormat(undefined, {}).format(columnTotals.OverWorkedCount) }}
            </td>
            <td colspan="2" class="text-right border">
              {{ Intl.NumberFormat(undefined, {}).format(columnTotals.LessWorkedCount) }}
            </td>
            <td colspan="2" class="text-right border">
              {{ Intl.NumberFormat(undefined, {}).format(columnTotals.TotalCount) }}
            </td>
          </tr>
        </tbody>
