...
const isByRegionAllowed = ref(auth.permissions.includes("ElderlyVerificationCountReportViewByRegion"));
const isByDistrictAllowed = ref(auth.permissions.includes("ElderlyVerificationCountReportViewByDistrict"));
...
const refresh = () => {
  ...
  ReportService.GetElderlyVerificationCount(innerFilter)
    .then((res: any) => {
...
const printReport = () => {
  ...
  ReportService.ExportElderlyVerificationCount(filter)
    .then((res: any) => {
      printLoading.value = false;
      forceFileDownload(res, t("GetElderlyVerificationCount"), ".xlsx");
    })
    .catch((err: any) => {
      setError(err);
      printLoading.value = false;
        ...
        <tbody v-if="!loading && rows.length > 0">
          <tr v-for="(item, index) in rows" :key="index + 'row'">
            ...
            <td colspan="2" class="text-right border">
              {{ Intl.NumberFormat(undefined, {}).format(item.processCount) }}
            </td>
            <td colspan="2" class="text-right border">
              {{ Intl.NumberFormat(undefined, {}).format(item.conclusionCount) }}
            </td>
            <td colspan="2" class="text-right border">
              {{ Intl.NumberFormat(undefined, {}).format(item.decisionCount) }}
            </td>
          ...
          <tr class="total">
            ...
            <td colspan="2" class="text-right border">
              {{ Intl.NumberFormat(undefined, {}).format(columnTotals.ProcessCount) }}
            </td>
            <td colspan="2" class="text-right border">
              {{ Intl.NumberFormat(undefined, {}).format(columnTotals.ConclusionCount) }}
            </td>
            <td colspan="2" class="text-right border">
              {{ Intl.NumberFormat(undefined, {}).format(columnTotals.DecisionCount) }}
            </td>
            <td colspan="2" class="text-right border">
