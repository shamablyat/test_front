...
const isByRegionAllowed = ref(auth.permissions.includes("OutReasonReportReportByRegion"));
const isByDistrictAllowed = ref(auth.permissions.includes("OutReasonReportReportByDistrict"));
...
const refresh = () => {
  ...
  ReportService.GetElderlyManageOutReasonsCount(innerFilter)
    .then((res: any) => {
...
const printReport = () => {
  ...
  ReportService.ExportElderlyManageOutReasonsCount(filter)
    .then((res: any) => {
      printLoading.value = false;
      forceFileDownload(res, t("GetElderlyManageOutReasonsCount"), ".xlsx");
    })
    .catch((err: any) => {
      setError(err);
      printLoading.value = false;
      ...
      <template #menu-items>
        <v-row class="mx-2">
          <v-col cols="12" md="12" sm="12" class="py-0 my-2">
            <form-select v-if="isByRegionAllowed" v-model="filter.regionId" multiple :list="regionsList" :label="$t('region')"> </form-select>
          </v-col>
